    ...
    <div className="fixed inset-0 z-[85] flex items-start justify-center overflow-y-auto bg-slate-950/45 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex items-start justify-between gap-5 border-b border-yearbook-line bg-yearbook-blue/45 px-5 py-5 sm:px-7">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-yearbook-muted">
              {archive.length
                ? '根据年鉴关联、题材交集、评分与人气重新排序；你标记喜欢与不喜欢的作品也会影响结果。'
                : '先从本季高分作品中挑一部，收录后这里会逐步长成你的专属推荐。'}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="p-5 sm:p-7">
          {loading && (
            <p className="py-16 text-center text-sm text-yearbook-muted">正在翻阅你的年鉴，寻找下一部作品...</p>
          )}
          {error && (
            <p className="mb-5 border-l-2 border-yearbook-pink bg-rose-50 px-3 py-2 text-sm text-yearbook-ink">
              {error}
            </p>
          )}
          {!loading && recommendations.length === 0 && (
            <p className="py-16 text-center text-sm text-yearbook-muted">推荐资料正在整理，稍后再回来看看。</p>
          )}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 py-1">
                    ...
                    <p className="mt-1 text-[11px] text-yearbook-muted">
                      {anime.seasonYear || '年份未知'} ·{' '}
                      {anime.genres.slice(0, 2).join(' / ') || anime.format || '动画'}
                    </p>
                    <p className="mt-3 line-clamp-3 text-xs leading-5 text-yearbook-muted">{reason}</p>
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <span className="text-xs font-medium text-yearbook-sky">
                        {anime.averageScore ? `${anime.averageScore}%` : '待评分'}
                      </span>
                      <button
                        type="button"
                        onClick={() => onToggle(anime)}
                        className={`text-sm font-medium transition ${selected ? 'text-yearbook-pink' : 'text-yearbook-sky hover:text-yearbook-ink'}`}
                      >
                        {selected ? '已收录' : '收录'}
                      </button>
                    </div>
