...
const titleAliases = (anime: Anime) =>
...
const titleMatchesArchive = (title: string, archiveAliases: string[]) => {
  ...
  return archiveAliases.some(
    (alias) =>
      normalized === alias ||
      (Math.min(normalized.length, alias.length) >= 5 && (normalized.includes(alias) || alias.includes(normalized)))
  );
};
          ...
          {loading ? (
            ...
            <>
              <section className="border border-sky-100 bg-sky-50/55 p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="text-sm font-bold text-sky-600 transition hover:text-slate-900"
                  >
                    {isChatGptOpen ? '收起' : '使用 ChatGPT'}
                  </button>
                </div>
                {isChatGptOpen && (
                  <div className="mt-3 space-y-3 border-t border-sky-100 pt-3">
                    ...
                    <textarea
                      ...
                      aria-label="粘贴 ChatGPT 鉴赏档案 JSON"
                      placeholder="粘贴 ChatGPT 返回的 JSON"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        ...
                        className="bg-sky-500 px-3 py-2 text-sm font-bold text-white transition hover:bg-sky-600 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        导入 JSON
                      </button>
                    </div>
              ...
              <div className="bg-white/75 rounded-2xl p-6 border border-rose-100 shadow-sm">
                <h3 className="text-lg font-black text-rose-500 mb-3 flex items-center gap-2">点评</h3>
              ...
              <div className="bg-white/75 rounded-2xl p-6 border border-sky-100 shadow-sm">
                <h3 className="text-lg font-black text-sky-600 mb-3 flex items-center gap-2">成分侧写</h3>
              ...
              <div className="bg-white/75 rounded-2xl p-6 border border-amber-100 shadow-sm">
                <h3 className="text-lg font-black text-amber-600 mb-3 flex items-center gap-2">避雷预警</h3>
                <div className="space-y-3">
                  {data.avoid.map((item, idx) => (
                    <div
                      key={`avoid-${idx}`}
                      className="flex flex-col gap-1 bg-amber-50/70 border border-amber-100 rounded-xl px-3 py-2"
                    >
              ...
              {/* Golden Era */}
              <div className="bg-white/75 rounded-2xl p-6 border border-sky-100 shadow-sm">
                <h3 className="text-lg font-black text-sky-600 mb-3 flex items-center gap-2">黄金年代</h3>
                <p className="text-slate-700 leading-relaxed">{data.goldenEra}</p>
              </div>

              {/* Recommendations */}
              <div>
                <h3 className="text-lg font-black text-slate-900 mb-4 flex items-center gap-2">补番推荐</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                  {visibleRecommendations.map((rec, idx) => (
                    <div
                      key={idx}
                      className="bg-white/75 rounded-2xl p-4 border border-sky-100 hover:border-sky-200 hover:bg-sky-50 transition-all group"
                    >
                      <div className="font-bold text-sky-700 mb-2 text-lg group-hover:text-rose-500 transition-colors">
                        {rec.title}
                      </div>
                ...
                {visibleRecommendations.length < 4 && (
                  <p className="mt-3 text-sm text-slate-500">
                    已过滤年鉴中已有的作品，剩余候选不足四部时可用 ChatGPT 协作模式补全。
                  </p>
                )}
              </div>
            </>
