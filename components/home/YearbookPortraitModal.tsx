...
export const YearbookPortraitModal: React.FC<YearbookPortraitModalProps> = ({ isOpen, onClose, year, anime, scope = 'year' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[86] flex items-center justify-center overflow-y-auto bg-slate-950/55 p-4 backdrop-blur-sm animate-fade-in">
      <section role="dialog" aria-modal="true" aria-labelledby="portrait-title" className="w-full max-w-3xl overflow-hidden border border-white/70 bg-yearbook-surface shadow-[0_28px_90px_rgba(38,54,77,0.28)]">
        ...
        <div className="grid gap-7 p-6 sm:grid-cols-[minmax(0,1fr)_220px] sm:p-8">
          <div>
            <div className="grid grid-cols-4 gap-2">
              {anime.slice(0, 8).map((item) => <img key={item.id} src={item.coverImage.large || item.coverImage.extraLarge} alt="" className="aspect-[3/4] w-full object-cover" loading="lazy" />)}
          ...
          <aside className="border border-yearbook-line bg-yearbook-paper/55 p-5">
            ...
            <dl className="mt-4 space-y-3">
              {(Object.keys(statusLabels) as UserAnimeStatus[]).map((status) => (
                <div key={status} className="flex items-center justify-between text-sm"><dt className="text-yearbook-muted">{statusLabels[status]}</dt><dd className="font-medium text-yearbook-ink">{statusCounts[status]}</dd></div>
              ))}
            </dl>
