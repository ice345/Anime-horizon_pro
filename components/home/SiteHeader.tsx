...
export const SiteHeader: React.FC<SiteHeaderProps> = ({
  ...
  const navItemClass = (active: boolean) => `min-h-11 px-2 text-sm font-medium transition-colors ${active ? 'text-yearbook-ink' : 'text-yearbook-muted hover:text-yearbook-ink'}`;
    ...
    <header className="relative z-30 border-b border-yearbook-line/80 bg-yearbook-paper/90 backdrop-blur-md">
      <div className="mx-auto flex min-h-[76px] max-w-[var(--ah-page-width)] items-center justify-between px-5 md:px-8">
        ...
        <div className="relative flex items-center gap-1">
          ...
          <button type="button" onClick={() => setMobileOpen((value) => !value)} aria-expanded={mobileOpen} aria-label="打开导航菜单" className="grid h-10 w-10 place-items-center rounded-full border border-yearbook-line text-yearbook-ink md:hidden">
            <MenuIcon />
          </button>
